...
namespace QuanLyNhanSu.QuanLy
{
    public partial class TinhLuong : UserControl
    {
        ...
        {
            if (string.IsNullOrEmpty(cbHeSoLuong.Text)) return;

            // Thử chuyển đổi sang int, tránh crash nếu Text không hợp lệ
            if (!int.TryParse(cbHeSoLuong.Text, out int a))
            {
                MessageBox.Show("Hệ số lương không hợp lệ!", "Lỗi",
            ...
            if (a > 0 && a < 11)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("Không tìm thấy lương cơ bản cho hệ số này!", "Thông báo",
                ...
                {
                    // Luôn đóng DataReader sau khi dùng xong
            ...
            else
            {
                txtLCB.Text = string.Empty;
                btCapNhat.Enabled = false;
                MessageBox.Show("Hệ số lương phải nằm trong khoảng từ 1 đến 10!", "Cảnh báo",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
